 ...
 * Date: 2018/10/23
 */
public class LngLat extends UDF {
    ...
    public static void main(String[] args) {
        is = LngLat.class.getClassLoader().getResourceAsStream("chinaBoudler.xml");
        try {
            List list=testRetrieveByCursor(120.965468,31.605479);
    ...
    public String evaluate(String px,String py) {
        try {
            is = LngLat.class.getClassLoader().getResourceAsStream("chinaBoudler.xml");
            if (StringUtils.isBlank(px) || StringUtils.isBlank(py)){
                return null;
            }
            double lng=Double.valueOf(px);
            double lat=Double.valueOf(py);
            List list=testRetrieveByCursor(lng,lat);
            if (list == null){
                return null;
            }
        ...
        while (streamReader.hasNext()) {
            ...
            switch (eventId) {
                ...
                case XMLStreamConstants.START_ELEMENT:
                    if (COUNTRY.equals(streamReader.getLocalName())) {
                        country = streamReader.getAttributeValue(2);
                    }
                    if (PROVINCE.equals(streamReader.getLocalName())) {
                        province = streamReader.getAttributeValue(2);
                    }
                    if (CITY.equals(streamReader.getLocalName()) && StringUtils
